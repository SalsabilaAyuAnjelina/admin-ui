describe("template spec", () => {
  it("passes", () => {
    ...
    cy.get('input[placeholder="Coffee"]', { timeout: 10000 }).should("be.visible").type("Appetizer").should("have.value", "Appetizer");

    //menekan button send
    cy.get('button[type="submit"]').click();

    cy.get('[data-testid="datagrid"]').find('[data-testid="deleteButton"]').first().click();
  });
});
